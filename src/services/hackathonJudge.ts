import { CommitMetrics } from './commitTimeline';
...
export type LlmTimelineDecision = {
  repo: string;
  judgement: 'qualified' | 'disqualified';
  reason: string;
  flags: string[];
};

function buildPrompt(metrics: CommitMetrics, startIso: string, endIso: string) {
...
export async function judgeRepoTimeline(metrics: CommitMetrics, startIso: string, endIso: string): Promise<LlmTimelineDecision> {
  ...
  let judgement = (parsed?.judgement === 'disqualified' ? 'disqualified' : 'qualified') as 'qualified'|'disqualified';
  ...
  let flags = Array.isArray(parsed?.flags) ? parsed.flags.map(String).slice(0, 10) : [];
    ...
    if (judgement === 'disqualified') {
      ...
      flags = flags.filter((f: string) => !/pre[_-]?hack/i.test(f) && !/pre[_-]?start/i.test(f) && !/disqual/i.test(f));
      judgement = 'qualified';
    ...
    logger.warn({ err }, 'LLM timeline judge failed, defaulting');
...
export default judgeRepoTimeline;
