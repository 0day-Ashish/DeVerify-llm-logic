export type ReadmeEval = {
  ...
  helpful: {
    ...
    techList: boolean;
    testsOrCI: boolean;
    licenseOrContrib: boolean;
    formatting: boolean;
  };
  secretsFound: boolean;
  base: number;    // 0 or 40
  extras: number;  // up to 60
  total: number;   // base + extras, capped at 100
  label: 'great' | 'okay' | 'needs-improvement';
  notes: string[];
};

export function evaluateReadme(readmeRaw: string | null | undefined): ReadmeEval {
  ...
  if (hasReadme) {
    ...
    for (let i = 0; i < Math.min(lines.length, 40); i++) {
      ...
      if (!ln) continue;
      if (ln.startsWith('#')) continue; // heading
      if (ln.startsWith('![') || ln.startsWith('<img')) continue; // image/badge
      // skip badges
      if (/\[!\[.*?\]\(.*?\)\]\(.*?\)/.test(ln)) continue;
  ...
  let extras = 0;
  if (install) extras += 10;
  if (usage) extras += 10;
  if (demoOrShot) extras += 10;
  if (techList) extras += 8;
  if (testsOrCI) extras += 6;
  if (licenseOrContrib) extras += 6;
  if (formatting) extras += 10;
  if (extras > 60) extras = 60;

  let total = base + extras;
  if (total > 100) total = 100;

  let label: ReadmeEval['label'] = 'needs-improvement';
  if (total >= 85) label = 'great';
  else if (total >= 60) label = 'okay';

  const notes: string[] = [];
  if (!hasReadme) notes.push('readme_missing_or_empty');
  if (hasReadme && !summary) notes.push('missing_one_line_summary');
  if (hasReadme && !howToRun) notes.push('missing_how_to_run');
  if (secretsFound) notes.push('secrets_like_strings_detected');
