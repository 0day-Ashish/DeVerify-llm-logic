...
export type LlmReadmeDecision = {
  score: number; // 0-100
  label: 'great' | 'okay' | 'needs-improvement';
  required: { summary: boolean; howToRun: boolean; hasReadme: boolean };
  helpful: { install: boolean; usage: boolean; demoOrShot: boolean; techList: boolean; testsOrCI: boolean; licenseOrContrib: boolean; formatting: boolean };
  notes: string[];
};

function buildPrompt(readme: string) {
...
export async function judgeReadmeWithLLM(readme: string): Promise<LlmReadmeDecision | null> {
  ...
  try {
    ...
    const label: LlmReadmeDecision['label'] = (parsed?.label === 'great' || parsed?.label === 'okay') ? parsed.label : 'needs-improvement';
    const required = {
    ...
    const notes: string[] = Array.isArray(parsed?.notes) ? parsed.notes.map(String).slice(0, 10) : [];
    return { score, label, required, helpful, notes };
  } catch (err) {
    logger.warn({ err }, 'LLM README judge failed');
    return null;
  }
}

export default judgeReadmeWithLLM;
