...
exports.verifyMint = verifyMint;
const ethers_1 = require("ethers");
const config_1 = require("../config");
async function verifyMint(evidence) {
    if (!evidence || !evidence.txHash)
        return { ok: false, reason: 'no_tx' };
    const rpc = config_1.config.alfajoresRpc;
    if (!rpc)
    ...
    try {
        ...
        const transferLog = (receipt.logs || []).find((l) => l.topics && l.topics[0] === transferTopic);
        if (!transferLog)
            return { ok: false, reason: 'no_transfer_event' };
        // Minimal decode - ethers v6 requires ABI
        const iface = new ethers_1.ethers.Interface([`event Transfer(address indexed from, address indexed to, uint256 indexed tokenId)`]);
        const decoded = iface.parseLog(transferLog);
        if (!decoded)
            return { ok: false, reason: 'parse_log_failed' };
        const tokenId = decoded.args?.tokenId?.toString?.() || evidence.tokenId || null;
        const to = decoded.args?.to || null;
        // Optional tokenURI check
        let metadataOk = null;
        if (evidence.tokenURI && evidence.contractAddress) {
            try {
                const erc = new ethers_1.ethers.Contract(evidence.contractAddress, [
                    'function tokenURI(uint256) view returns (string)'
                ], provider);
                const onchain = await erc.tokenURI(tokenId);
                if (onchain !== evidence.tokenURI)
                    metadataOk = false;
                else
                    metadataOk = true;
            }
