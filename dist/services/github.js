...
async function fetchGitHubMetadata(repoUrl) {
    ...
    function computeReadmeInfo(md) {
        ...
        for (const [re, tag] of boilerplatePatterns) {
            if (re.test(text))
                boilerplateHits.push(tag);
        }
        const minimal = length < 250 && headingCount === 0 && !hasInstall && !hasUsage;
        const richnessScore = ((length >= 1000 ? 2 : length >= 400 ? 1 : 0) +
    ...
    try {
        const r = await axios_1.default.get(`${base}/repos/${owner}/${repo}/readme`, { headers });
        if (r?.data?.content) {
    ...
    // repo details
    try {
        const r = await axios_1.default.get(`${base}/repos/${owner}/${repo}`, { headers });
        out.size = r.data.size; // in KB roughly
        out.fork = r.data.fork;
        out.languages = (await axios_1.default.get(r.data.languages_url, { headers })).data;
    ...
    // commits - latest
    try {
        const r = await axios_1.default.get(`${base}/repos/${owner}/${repo}/commits`, { headers, params: { per_page: 1 } });
    ...
    // contributors count
    try {
        const r = await axios_1.default.get(`${base}/repos/${owner}/${repo}/contributors`, { headers, params: { per_page: 1, anon: true } });
    ...
    try {
        if (out.latestCommitSha) {
            const r = await axios_1.default.get(`${base}/repos/${owner}/${repo}/commits/${out.latestCommitSha}/status`, { headers });
    ...
    try {
        ...
        for (const f of files) {
            try {
                const resp = await axios_1.default.get(`${base}/repos/${owner}/${repo}/contents/${f}`, { headers });
                if (resp?.data)
                    manifestHits.push(f);
            }
            catch (e) {
