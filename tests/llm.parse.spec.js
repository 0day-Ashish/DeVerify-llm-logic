...
jest.mock('openai', () => {
    const create = jest.fn().mockImplementation(() => {
        ...
        if (callIndex === 1)
            return { choices: [{ message: { content: validJson } }] };
        // 2: malformed then 3: valid for retry test
        if (callIndex === 2)
            return { choices: [{ message: { content: 'I think this is fine...' } }] };
        if (callIndex === 3)
            return { choices: [{ message: { content: validJson } }] };
...
describe('llm.parse and fallback', () => {
    ...
    it('scoreRepo retries on malformed then succeeds', async () => {
        ...
        const res = await scoreRepo(baseRepo, baseSignals, { present: false });
    ...
    it('scoreRepo returns deterministic fallback after two malformed responses', async () => {
        ...
        callIndex = 100;
        const out = await scoreRepo(baseRepo, baseSignals, { present: false });
